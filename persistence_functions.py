...
def get_persistence(state_file,states_to_analyze={1:'warm',-1:'cold'}, lat_name='lat', lon_name='lon', seasons={'MAM':{'months':[3,4,5],'index':0}, 'JJA':{'months':[6,7,8],'index':1}, 'SON':{'months':[9,10,11],'index':2}, 'DJF':{'months':[12,1,2],'index':3}},overwrite=True):
	...
	for state_id,state_name in states_to_analyze.items():
		...
		ds_out['period_length'] = da.DimArray(period_length[0:per_num,:,:], axes=[np.asarray(range(per_num),dtype=np.dtype('i2')),nc_in['lat'].values,nc_in['lon'].values], dims=['period_id','lat','lon'], dtype=np.dtype('i2'))
		...
		ds_out['period_midpoints'] = da.DimArray(period_midpoints[0:per_num,:,:], axes=[np.asarray(range(per_num),dtype=np.dtype('i2')),nc_in['lat'].values,nc_in['lon'].values], dims=['period_id','lat','lon'], dtype=np.dtype('f'))
		...
		ds_out['period_season'] = da.DimArray(period_season[0:per_num,:,:], axes=[np.asarray(range(per_num),dtype=np.dtype('i2')),nc_in['lat'].values,nc_in['lon'].values], dims=['period_id','lat','lon'], dtype=np.dtype('i1'))
		...
		ds_out['period_monthly_index'] = da.DimArray(period_monthly_index[0:per_num,:,:], axes=[np.asarray(range(per_num),dtype=np.dtype('i2')),nc_in['lat'].values,nc_in['lon'].values], dims=['period_id','lat','lon'], dtype=np.dtype('i2'))
